...
# I'm just going to use the Raw data and use my current pipeline...
...
targetblotse<-function(lab,interval1,interval2,tabular=FALSE){
  ...
  cdf <- do.call(rbind,
    ...
    g<-ggplot(data=subset(aa,Lab==lab))+
      ...
      stat_ellipse(data=subset(ellipse,Lab==lab),aes(x=value,y=value2,col=Tissue,fill=Tissue),geom="polygon",alpha=0.7)+
      scale_fill_manual(values = c("blue","yellow", "red"))+
      labs(x="Input Est. Mix1", y="Input Est. Mix2", title = paste(" "))+
      theme(legend.position="none")+
      coord_fixed()+geom_path(data=cdf,alpha=0.5,aes(group=factor(r),x=x,y=y,col=r))+
      scale_color_manual(values = c("black","blue","blue","blue","black","orange","orange","orange","black","red","red","red"))+
      geom_segment(aes(x=pointdata["Liver"][1,],xend=expectation["mix1"][2,],y=pointdata["Liver"][2,],yend=expectation["mix2"][2,],col="Liver"), linetype = "longdash", color="orange")+
      geom_segment(aes(x=pointdata["Brain"][1,],xend=expectation["mix1"][1,],y=pointdata["Brain"][2,],yend=expectation["mix2"][1,],col="Brain"), linetype = "longdash", color="blue")+
      geom_segment(aes(x=pointdata["Placenta"][1,],xend=expectation["mix1"][3,],y=pointdata["Placenta"][2,],yend=expectation["mix2"][3,],col="Placenta"), linetype = "longdash", color="red")
    return(g)
}
